...
func Range[T ordered](start, endExclusive T) (out []T) {
...
func ReduceWithError[S ~[]InType, InType any, OutType any](in S, fn func(OutType, InType) (OutType, error), out OutType) (final OutType, err error) {
	for i := range in {
		final, err = fn(out, in[i])
		if err != nil {
			return final, err
		}
	}
...
func Any[T any](in []T, fn func(in T) bool) bool {
	for i := range in {
		if fn(in[i]) {
			return true
		}
...
func First[S ~[]T, T any](in S, fn func(T) bool) (t T, i int, ok bool) {
	for i := range in {
		ok := fn(in[i])
		if ok {
			return in[i], i, true
...
func GroupBy[T any, U comparable](stuff []T, discriminator func(T) U) [][]T {
	var groups [][]T
	var prev U
	for _, thing := range stuff {
		current := discriminator(thing)
		if current != prev {
			groups = append(groups, []T{thing})
			prev = current
		} else {
			groups[len(groups)-1] = append(groups[len(groups)-1], thing)
		}
	}
	return groups
}
